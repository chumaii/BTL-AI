...
def GetNews(limit_news=20):  # Tạo biến limit_news để lấy số lượng tin mà mình cần thôi
    ...
    for element in article:
        ...
        for x in range(len(title)):  # serialize object này lại thành json để lấy dữ liệu dễ dàng hơn
            listArticle.append(json.dumps(Article(title[x]['href'], description[x].text).__dict__,
                                          ensure_ascii=False))
    return listArticle
# Hàm này được sử dụng để lấy thông tin tin tức từ trang web và trả về một danh sách các đối tượng bài báo (Article object) dưới dạng JSON string. 
